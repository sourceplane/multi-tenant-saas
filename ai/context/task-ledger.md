...
Last updated: 2026-05-30 (Task 0091 Verifier PASS — PR #139 merged at `9081cff`, post-merge main-CI `26668839091` 3/3 SUCCESS, workspace-wide `pnpm -r typecheck` exits 0 cleanly for the first time in repo history; Task 0092 SCOPED — ESLint v9 flat-config scaffold for the 16 lint-bearing workspaces missing `eslint.config.js`)
...
- Status: scoped 2026-05-30, ready to begin (orchestrator output while Track A waits on implementer fix-up to PR #143)
- Objective: drive `pnpm --filter tests/membership-worker lint` warning count from 350 → 0 by replacing every `@typescript-eslint/no-explicit-any` site in `tests/membership-worker/src` with the narrowest accurate type (preferring real exports from `@saas/contracts`, `@saas/db/membership`, and `apps/membership-worker/src/**`), preserving behaviour, suite ordering, and assertion semantics.
- Scope boundary (what's in): edits inside `tests/membership-worker/src/**/*.ts` only (5 files: `membership-worker.test.ts` 3637 LOC dominates with ~330 of the 350 warnings; `accept-invitation-notifications.test.ts`, `authorization-context.test.ts`, `create-invitation-notifications.test.ts`, `service-principal-bindings.test.ts`); optional in-workspace `_types.ts` for shared fixture types; `ai/reports/task-0096b-implementer.md` (NEW).
- Scope boundary (what's out): no edits under `apps/**`, `packages/**`, `infra/**`, `tooling/**`, `.github/**`, `specs/**`, or any other `tests/<workspace>/**`; no `eslint-disable*` / `@ts-ignore` / `@ts-expect-error` / `as unknown as` introductions in the diff; no rule severity flip from `warn` to `error`; **no touching of PR #143 surface (`apps/api-edge/**`, `infra/terraform/cloudflare-kv/**`, `tests/api-edge/**`)** so this PR cannot conflict with the Track A rebase.
- Acceptance: `pnpm --filter tests/membership-worker lint` exit 0 with **0 warnings** (was 350); `pnpm --filter tests/membership-worker test` green with suite + test count UNCHANGED vs `main` @ `d2187f1`; `pnpm -r typecheck` exit 0 (Task 0091 baseline); `pnpm -r --no-bail lint` exit 0 with **277 residual warnings** (627 − 350), all in `tests/**` other workspaces, apps-source still 0 (Task 0096 invariant); hazard scan empty (`git diff origin/main -- 'tests/membership-worker/**' | grep -E '^\+.*(eslint-disable|@ts-(ignore|expect-error)|as unknown as)'` → no output); PR opened on branch `impl/task-0096b-tests-membership-worker-class-b` with real PR number written into the report (no `TBD` / `#[PR]` placeholders after final push).
- Expected outcome: `tests/membership-worker` becomes the second `tests/**` workspace at 0 class-B warnings (after the apps-source clean shipped in Task 0096), shrinking the residual lint surface from 627 → 277, and establishing the wave-2 pattern (single-workspace PR, real types from worker exports / @saas/contracts / @saas/db/*) for the remaining seven workspaces (`tests/config-worker` 126, `tests/identity-worker` 80, `tests/api-edge` 45, `tests/projects-worker` 10, `tests/policy-engine` 7, `tests/events-worker` 7, `tests/policy-worker` 1, `tests/webhooks-worker` 1) which become subsequent waves.
- Recommended next move on PASS + merge: if Track A (Task 0095.1) implementer fix-up has landed by then, run `ai/tasks/task-0095.1-verifier.md`; otherwise the next class-B wave (largest remaining: `tests/config-worker` 126) or Task 0097 rate-limiting (gated on Track A merging because it reuses the `cloudflare-kv` slice from Task 0095).
