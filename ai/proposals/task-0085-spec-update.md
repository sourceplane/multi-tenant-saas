...
**Status:** ACCEPTED by orchestrator on 2026-05-29.

The orchestrator accepts the rescope. Reasoning:

- The two failing PR-CI runs (`26642692516` bare `moved{}`,
  `26642904336` `removed{}+import{}` under `~> 5.0`) are concrete,
  reproducible evidence that no single-PR shape is available with
  the current cloudflare provider v5.
- The two-phase pattern this proposal describes is exactly the
  shape the official v5 upgrade guide prescribes for the
  `cloudflare_zone_settings_override` rename, and exactly what
  `tf-migrate` automates for this class of cross-type rename. It is
  the lowest-risk path that keeps each PR's plan diff strictly
  state-only and reviewable in one pass.
- The single-PR alternative (let Terraform plan `1 to destroy, 1
  to add` and accept the brief unbind) was rejected because it
  would briefly take `stage.sourceplane.ai` and
  `prod.sourceplane.ai` off their Workers on apply — a real,
  user-visible outage on the only two live custom-domain endpoints
  in the SaaS.

**Resulting task split:**

- **Task 0085a** — `ai/tasks/task-0085a.md` (queued for
  implementer). Phase 1: under `cloudflare ~> 4.52`, add `removed
  { from = cloudflare_workers_domain.console; lifecycle { destroy
  = false } }`, fence/comment-out the live v4 `resource` block.
  Expected plan diff: `Plan: 0 to add, 0 to change, 1 to forget.`
  on both envs. Zero Cloudflare API writes.
- **Task 0085b** — to be scoped by the orchestrator AFTER 0085a
  is verified, merged, AND the post-merge `forgotten` apply has
  landed on both stage and prod. Phase 2: bump
  `required_providers.cloudflare.version` to `~> 5.0`, replace the
  fenced v4 resource with the v5
  `cloudflare_workers_custom_domain.console`, add `import {}`
  blocks keyed by env that re-adopt by the known immutable IDs
  (stage `052eaece5e989d5a7280b6c206e562c42950e3a6`, prod
  `31e5f2ed1b1e4a5700e8ae0678846a0d753840e1`).

**PR #132 disposition:** to be closed (NOT merged) as superseded
by the implementer of 0085a, with a one-line comment pointing to
the new 0085a PR. The branch can be left for reference; no rebase
or salvage from it is required (0085a starts a fresh branch off
`main`).

**Live invariant across both phases:** the two custom-domain
resource IDs must survive byte-identical. This proposal exists to
