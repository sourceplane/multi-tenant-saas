...
| **BF** | [`epics/saas-bootstrap-factory/`](./epics/saas-bootstrap-factory/) | Partly shipped; D–E superseded by **LB** | Make the starter instantiable: BF0–BF2 truth + typed params · BF3–BF6 config indirection + deploy-time wiring (no committed resource IDs) · BF7–BF9 domain/foundation/preflight · BF10–BF12 OCI stack + Blueprint/Instance contracts + instantiator · BF13–BF14 acme rehearsal + upgrade path. **BF10–BF14 are superseded by [`saas-lumen-baseline`](./epics/saas-lumen-baseline/)**, which adopts Lumen's proven phased bootstrap instead of building a second instantiator. |
| **LB** | [`epics/saas-lumen-baseline/`](./epics/saas-lumen-baseline/) | In progress | Make this repo a baseline the platform can build from: LB0 platform-posture convergence · LB1 brokered credentials + Orun state backend (keystone) · LB2 component contract parity · LB3 the `flows/` bootstrap + blueprints · LB4 component docs trio · LB5 front door · LB6 registration, release tag, verified instantiation. |
...
| **SS** | [`epics/saas-secrets-sync/`](./epics/saas-secrets-sync/) | **Closed — superseded by LB1** | One write path for every secret: SS0 escrow convention + manifest · SS1 drift checker enforced in verify lanes · SS2 deploy-lane sync · SS3 escrow seeding (human-gated) · SS4 Secrets Store for shared keys · SS5 rotation runbook + BF9 preflight. Built on the AWS Secrets Manager escrow that **LB1** removes from the loop; see [`saas-lumen-baseline/design.md` §1.5](./epics/saas-lumen-baseline/design.md). |
