...
| BP2.2 — native subscription cancel | ✅ Shipped | — | Cancel happens in-app via the Polar Customer Portal API (`customerPortal.subscriptions.cancel`, customer-session token) — no hosted-portal redirect. The downgrade still flows through the verified webhook; the console polls until it lands. Updating the payment method remains a deep-link to the hosted portal (PCI). Polar's hosted portal is intentionally non-white-label, so management UI is native. |
