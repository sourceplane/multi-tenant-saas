...
| [`saas-multi-org-billing/`](./saas-multi-org-billing/) | **B** (billing platform) | Draft (not started) | membership-worker, billing-worker, db, console | Datadog-style multi-org: default single org, more orgs are a purchased capability, billing rolls up to the default/parent org. Carries the `billing-provider-abstraction` sub-epic (Polar first, Stripe/others by config). Extends B6 + B11. |
| [`saas-product-areas/`](./saas-product-areas/) | **P1, P3–P7** | Holding register | various | Promote-flow, observability, notification inbox, marketplace, changelog/status, AI-native affordances. Each promoted to its own folder when work starts. |
