...
| [`saas-lumen-baseline/`](./saas-lumen-baseline/) | **LB** | In progress | `flows/` (new), `infra/terraform/*`, all `component.yaml`, `intent.yaml`, `sourceplane/orun-cloud` | Make this repo a baseline the platform can build from: brokered credentials and Orun-backed state in place of the AWS escrow, the phased bootstrap Lumen proved, docs on every component, and a registered entry on `/baselines`. Supersedes `saas-bootstrap-factory` D–E and `saas-secrets-sync`. |
