...
```text
sourceplane/multi-tenant-saas/supabase/dev
```
...
- `multi-tenant-saas` uses the same Orun runtime version as `aws-admin`.
...
- Supabase database credentials are generated by Terraform and stored in AWS
  Secrets Manager.
