# events-worker — architecture

A `cloudflare-worker-turbo` component: TypeScript Worker built by the turbo pipeline from `apps/events-worker`, deployed per environment by its CI lane.
