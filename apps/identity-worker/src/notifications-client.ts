 ...
 * primary auth lifecycle.
 *
 * No secret material (raw codes, tokens, provider responses) MUST be
 * placed in `templateData`. Allowed values are a bounded redaction-safe
 * subset (presentation hints + the code itself, which is the message
 * payload, not a secret of the auth state — the codeHash is what is
...
export interface NotificationsClientContext {
  /** Caller identifier — for identity-worker this is `"identity-worker"`. */
