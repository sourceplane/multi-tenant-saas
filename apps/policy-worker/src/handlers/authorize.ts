...
export async function handleAuthorize(request: Request, requestId: string): Promise<Response> {
  ...
  const validation = validateAuthorizeBody(body);
  if (validation) {
    return validationError(requestId, validation);
  }

  const input = body as AuthorizationRequest;
  const result = authorize(input);

  return successResponse(result, requestId);
}

function validateAuthorizeBody(body: unknown): Record<string, string[]> | null {
