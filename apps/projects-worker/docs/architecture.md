# projects-worker — architecture

A `cloudflare-worker-turbo` component: TypeScript Worker built by the turbo pipeline from `apps/projects-worker`, deployed per environment by its CI lane.

## Bindings and wiring

- **Service bindings** → `billing-worker`, `membership-worker`, `policy-worker` — in-process RPC to sibling Workers; no public hops between contexts.
