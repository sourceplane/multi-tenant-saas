...
The single public entry point. Every request from outside enters here, is authenticated and rate-limited, and is routed to the owning bounded-context Worker over a service binding. Nothing else in the fleet is publicly routable.

A Cloudflare Worker deployed per environment (`stage`, `prod`; `dev` is verify-only), public at `https://api-edge-{stage,prod}.rahulvarghesepullely.workers.dev`.
