...
- **"Resource already exists" with empty platform state**: adoption (`adopt.tf`) imports at plan time. If a root lacks adoption the resource must be state-migrated or deleted.
- **Provider auth failure**: the workspace's integration connection is missing or revoked. Reconnect it in the console — credentials are brokered from it per run, so nothing here is repairable by editing the repo.
