...
|----------|-------------|---------|
| `BASE_DOMAIN` | Root domain for the zone | `multi-tenant-saas.app` |
| `CONSOLE_CUSTOM_DOMAIN` | Custom domain for the console Worker | `stage.multi-tenant-saas.app` |
...
|-----------|----------|-------------|
| `baseDomain` | yes | Root domain (e.g. `multi-tenant-saas.app`) |
| `zoneMode` | yes | `existing` or `managed` |
| `workerNamePrefix` | yes | Worker name prefix (e.g. `multi-tenant-saas-web-console-next`); fully-qualified service name is `{workerNamePrefix}-{environment}` |
...
2. Workers custom domains show `active` status in Cloudflare (Workers → the
   `multi-tenant-saas-web-console-next-{env}` Worker → Triggers → Custom Domains).
3. `https://stage.multi-tenant-saas.app/` serves the stage console (web-console-next).
4. `https://prod.multi-tenant-saas.app/` serves the prod console (web-console-next).
